    ...
    <section>
      ...
      <v-card>
        <v-card-title>
          ...
          <span class="headline px-5">Informasi Jadwal {{ skemas.skema }}</span>
          <v-spacer></v-spacer>
        </v-card-title>
        <v-divider></v-divider>
        <v-row>
          <v-col cols="12" class="px-10 py-5">
            <div class="px-5 py-5">
              ...
              <template v-slot:item.skema="{ item }">
...
import { GET_JADWALS_BYSKEMA, GET_ANGGARANS } from '@/constants/graphql';
  ...
  export default {
    ...
    data() {
      return {
        skema_id: this.$route.params.jadwal,
